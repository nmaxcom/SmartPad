...
This document defines SmartPad's first persistent comparison workflow: capture the current numeric model as a baseline, manipulate the sheet, and read live changes beside the variables.
...
2. Choose `Set baseline` in the Variables panel.
3. Edit or scrub numbers in the sheet.
4. Read the changed-variable count and per-variable difference from the captured values.
5. Choose `Update` to make the current model the new baseline, or `Clear` to stop comparing.
...
The labels explain model structure; they do not alter evaluation or prevent editing.
...
- Unchanged variables display `same`.
...
- Without a baseline, the Variables panel offers `Set baseline` and the hint `Capture, scrub, compare.`
- With a baseline, it shows `Compared with baseline`, the number of matched variables that changed, and `Update` / `Clear` actions.
- Each captured current variable shows `Base <captured value>` and its percent delta or change state.
- Changed rows receive a lightweight accent; the comparison must remain readable without relying on color alone.
...
After capture, `ticket price` is `input`, `revenue` is `derived`, and the summary is `0 changed`.

After changing `ticket price` to `40 EUR`:

- its baseline line remains `Base 32 EUR`
- its delta is `+25%`
- `revenue` also changes from its baseline
- the changed count includes both matched variables

After `Update`, the summary returns to `0 changed`. Reloading retains the comparison; `Clear` removes it.
