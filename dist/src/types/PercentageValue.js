 ...
 */
class PercentageValue extends SemanticValue_1.SemanticValue {
    displayPercentage; // What user sees (20 for 20%)
    decimalValue; // Calculation value (0.2 for 20%)
    context;
    constructor(displayPercentage, context = 'standalone') {
        super();
    ...
    add(other) {
        if (other.getType() === 'percentage') {
            // 20% + 30% = 50%
            const otherPercent = other.getDisplayPercentage();
        ...
        if (other.getType() === 'number') {
            ...
            return new NumberValue_1.NumberValue(percentAsNumber + other.getNumericValue());
        }
    ...
    divide(other) {
        ...
        if (other.getType() === 'percentage') {
            ...
            return new NumberValue_1.NumberValue(result);
        }
    ...
    on(baseValue) {
        if (baseValue.getType() === 'number') {
            ...
            return new NumberValue_1.NumberValue(baseValue.getNumericValue() + increase);
        }
    ...
    off(baseValue) {
        if (baseValue.getType() === 'number') {
            ...
            return new NumberValue_1.NumberValue(baseValue.getNumericValue() - decrease);
        }
        if (baseValue.getType() === 'currency' || baseValue.getType() === 'unit' || baseValue.getType() === 'currencyUnit') {
     ...
     */
    toDecimal() {
        return new NumberValue_1.NumberValue(this.decimalValue);
    }
    /**
     ...
     */
    static withContext(displayPercentage, context) {
     ...
     */
    static whatPercentOf(part, whole) {
