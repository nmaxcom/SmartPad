...
exports.UnitsParser = exports.UnitsTokenType = void 0;
exports.aliasVariablesInExpression = aliasVariablesInExpression;
exports.tokenizeWithUnits = tokenizeWithUnits;
exports.evaluateUnitsExpression = evaluateUnitsExpression;
exports.expressionContainsUnits = expressionContainsUnits;
const quantity_1 = require("./quantity");
/**
 * Enhanced token types for units
 */
var UnitsTokenType;
(function (UnitsTokenType) {
    // Existing token types
    UnitsTokenType["NUMBER"] = "NUMBER";
    UnitsTokenType["OPERATOR"] = "OPERATOR";
    UnitsTokenType["FUNCTION"] = "FUNCTION";
    UnitsTokenType["IDENTIFIER"] = "IDENTIFIER";
    UnitsTokenType["LEFT_PAREN"] = "LEFT_PAREN";
    UnitsTokenType["RIGHT_PAREN"] = "RIGHT_PAREN";
    UnitsTokenType["COMMA"] = "COMMA";
    UnitsTokenType["EOF"] = "EOF";
    // New token types for units
    UnitsTokenType["UNIT"] = "UNIT";
    UnitsTokenType["QUANTITY"] = "QUANTITY";
})(UnitsTokenType || (exports.UnitsTokenType = UnitsTokenType = {}));
/**
 ...
 */
function tokenizeWithUnits(expression) {
    const tokens = [];
    let position = 0;
    ...
    while (position < expression.length) {
        ...
        if (/\d/.test(char) || char === ".") {
            ...
            if (position < expression.length && /[a-zA-Z°]/.test(expression[position])) {
                ...
                try {
                    const unit = quantity_1.UnitParser.parse(unitStr);
                    const value = parseFloat(numberStr);
                    const quantity = new quantity_1.Quantity(value, unit);
                    tokens.push({
...
class UnitsParser {
    ...
    parseExpression() {
        ...
        while (this.currentToken().type === UnitsTokenType.OPERATOR &&
    ...
    parseTerm() {
        ...
        while (this.currentToken().type === UnitsTokenType.OPERATOR &&
            ...
            if (operator === "*") {
                ...
                left = quantity_1.Quantity.dimensionless(leftValue % rightValue);
            }
        }
        return left;
    }
    ...
    parsePrimary() {
        ...
        if (token.type === UnitsTokenType.OPERATOR && token.value === "-") {
            ...
            return new quantity_1.Quantity(-operand.value, operand.unit);
        }
        ...
        if (token.type === UnitsTokenType.NUMBER) {
            ...
            return quantity_1.Quantity.dimensionless(value);
        }
        // Functions
    ...
    evaluateFunction(name, args) {
        ...
        switch (lowerName) {
            case "sqrt":
                ...
                return quantity_1.Quantity.dimensionless(Math.sqrt(args[0].value));
            case "abs":
                if (args.length !== 1) {
                    throw new Error(`abs expects 1 argument, got ${args.length}`);
                }
                return new quantity_1.Quantity(Math.abs(args[0].value), args[0].unit);
            case "round":
                if (args.length !== 1) {
                    throw new Error(`round expects 1 argument, got ${args.length}`);
                }
                return new quantity_1.Quantity(Math.round(args[0].value), args[0].unit);
            case "floor":
                if (args.length !== 1) {
                    throw new Error(`floor expects 1 argument, got ${args.length}`);
                }
                return new quantity_1.Quantity(Math.floor(args[0].value), args[0].unit);
            case "ceil":
                if (args.length !== 1) {
                    throw new Error(`ceil expects 1 argument, got ${args.length}`);
                }
                return new quantity_1.Quantity(Math.ceil(args[0].value), args[0].unit);
            default:
                throw new Error(`Unknown function: ${name}`);
        }
    }
}
exports.UnitsParser = UnitsParser;
/**
 * Main evaluation function for units-aware expressions
 */
function evaluateUnitsExpression(expression, variables = {}) {
    try {
        ...
        return {
            quantity: quantity_1.Quantity.dimensionless(0),
 ...
 */
function expressionContainsUnits(expression) {
    try {
