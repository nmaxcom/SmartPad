...
exports.UnitsNetParser = exports.UnitsNetTokenType = void 0;
exports.tokenizeWithUnitsNet = tokenizeWithUnitsNet;
exports.evaluateUnitsNetExpression = evaluateUnitsNetExpression;
exports.expressionContainsUnitsNet = expressionContainsUnitsNet;
const unitsnetAdapter_1 = require("./unitsnetAdapter");
const types_1 = require("../types");
/**
 * Enhanced token types for units
 */
var UnitsNetTokenType;
(function (UnitsNetTokenType) {
    // Existing token types
    UnitsNetTokenType["NUMBER"] = "NUMBER";
    UnitsNetTokenType["OPERATOR"] = "OPERATOR";
    UnitsNetTokenType["FUNCTION"] = "FUNCTION";
    UnitsNetTokenType["IDENTIFIER"] = "IDENTIFIER";
    UnitsNetTokenType["LEFT_PAREN"] = "LEFT_PAREN";
    UnitsNetTokenType["RIGHT_PAREN"] = "RIGHT_PAREN";
    UnitsNetTokenType["COMMA"] = "COMMA";
    UnitsNetTokenType["EOF"] = "EOF";
    // New token types for units
    UnitsNetTokenType["UNIT"] = "UNIT";
    UnitsNetTokenType["QUANTITY"] = "QUANTITY";
    UnitsNetTokenType["CONSTANT"] = "CONSTANT";
    UnitsNetTokenType["TO"] = "TO";
})(UnitsNetTokenType || (exports.UnitsNetTokenType = UnitsNetTokenType = {}));
/**
 ...
 */
function aliasVariablesInExpression(expression, variables) {
    ...
    if (names.length === 0)
        return { expression: expr, variables };
 ...
 */
function tokenizeWithUnitsNet(expression) {
    const tokens = [];
    let position = 0;
    ...
    while (position < expression.length) {
        ...
        if (/\d/.test(char) || char === ".") {
            ...
            if (position < expression.length && unitStartRe.test(expression[position])) {
                ...
                try {
                    ...
                    const quantity = new types_1.UnitValue(unitsnetAdapter_1.SmartPadQuantity.fromValueAndUnit(value, unitStr));
                    tokens.push({
        ...
        // Constants (PI, E)
            ...
            if (constantStr in unitsnetAdapter_1.MATHEMATICAL_CONSTANTS) {
                tokens.push({
                    type: UnitsNetTokenType.CONSTANT,
                    value: constantStr,
                    position: constantStart,
                    constant: new types_1.NumberValue(unitsnetAdapter_1.MATHEMATICAL_CONSTANTS[constantStr]),
                });
            }
            else {
...
class UnitsNetParser {
    ...
    parseExpression() {
        ...
        while (this.currentToken().type === UnitsNetTokenType.OPERATOR &&
            ["+", "-"].includes(this.currentToken().value)) {
            const operator = this.consume().value;
            const right = this.parseTerm();
            if (operator === "+") {
                const result = left.add(right);
                if (result instanceof types_1.UnitValue || result instanceof types_1.NumberValue) {
                    left = result;
                }
                else {
        ...
        if (this.currentToken().type === UnitsNetTokenType.TO) {
            ...
            while (true) {
                ...
                if (t.type === UnitsNetTokenType.IDENTIFIER ||
            ...
            if (left instanceof types_1.UnitValue) {
    ...
    parseTerm() {
        ...
        while (this.currentToken().type === UnitsNetTokenType.IDENTIFIER && left instanceof types_1.NumberValue) {
            const unitId = this.consume().value;
            try {
                const quantity = unitsnetAdapter_1.SmartPadQuantity.fromValueAndUnit(left.getNumericValue(), unitId);
                left = new types_1.UnitValue(quantity);
            }
            catch {
                break;
            }
    ...
    parseFactor() {
        ...
        while (this.currentToken().type === UnitsNetTokenType.OPERATOR &&
            ...
            if (!(exponent instanceof types_1.NumberValue)) {
    ...
    parsePower() {
        ...
        if (value instanceof types_1.NumberValue) {
            const t = this.currentToken();
            if (t.type === UnitsNetTokenType.IDENTIFIER &&
                (t.value === "h" || t.value === "min" || t.value === "day")) {
                this.consume();
                try {
                    const quantity = unitsnetAdapter_1.SmartPadQuantity.fromValueAndUnit(value.getNumericValue(), t.value);
                    value = new types_1.UnitValue(quantity);
                }
                catch { }
            }
        }
    ...
    parseUnary() {
        if (this.currentToken().type === UnitsNetTokenType.OPERATOR &&
            ...
            if (operand instanceof types_1.NumberValue) {
                return new types_1.NumberValue(-operand.getNumericValue());
            }
            const negated = operand.multiply(new types_1.NumberValue(-1));
            if (negated instanceof types_1.UnitValue || negated instanceof types_1.NumberValue) {
                return negated;
            }
    ...
    parsePrimary() {
        ...
        switch (token.type) {
            case UnitsNetTokenType.NUMBER:
                this.consume();
                return new types_1.NumberValue(parseFloat(token.value));
    ...
    parseFunctionCall() {
        ...
        const smartPadArgs = args.map(arg => {
            if (arg instanceof types_1.UnitValue) {
                return arg.getQuantity();
            }
            else {
                return unitsnetAdapter_1.SmartPadQuantity.dimensionless(arg.getNumericValue());
            }
        });
        const result = unitsnetAdapter_1.UnitsNetMathEvaluator.evaluateFunction(functionName, smartPadArgs);
        return result.isDimensionless() ? new types_1.NumberValue(result.value) : new types_1.UnitValue(result);
    }
}
 ...
 */
function evaluateUnitsNetExpression(expression, variables = {}) {
    try {
        // Alias phrase variables to letter-only identifiers that the tokenizer accepts
        const { expression: aliasedExpr, variables: aliasedVars } = aliasVariablesInExpression(expression, variables);
        // Rewrite simple time literals like '1 h' into seconds to avoid tokenizer edge cases
        const rewriteSimpleTimeQuantities = (expr) => {
        ...
        if (aliasedExpr.includes("1 h")) {
            console.log("TOKENS FOR '1 h':", tokens.map((t) => `${t.type}:${t.value}`));
        }
        ...
        console.log("UnitsNet evaluate error:", error instanceof Error ? error.message : String(error), "expr:", expression);
        return {
            value: new types_1.NumberValue(0),
 ...
 */
function expressionContainsUnitsNet(expression) {
    return unitsnetAdapter_1.UnitsNetParser.containsUnits(expression);
}
