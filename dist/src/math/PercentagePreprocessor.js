...
exports.PercentagePreprocessor = void 0;
/**
...
class PercentagePreprocessor {
    ...
    static process(expression) {
        ...
        while (result !== lastResult) {
            ...
            result = result.replace(/(\d+(?:\.\d+)?)%\s*on\s+(\d+(?:\.\d+)?)%\s*off\s+(\S+)/g, '(($3) - (($2/100) * ($3))) + (($1/100) * (($3) - (($2/100) * ($3))))');
            // Process "X% off Y" -> Y - ((X/100) * Y)
            result = result.replace(/(\d+(?:\.\d+)?)%\s*off\s+(\([^)]+\)|\S+)/g, '$2 - (($1/100) * $2)');
            // Process "X% on Y" -> Y + ((X/100) * Y) 
            result = result.replace(/(\d+(?:\.\d+)?)%\s*on\s+(\([^)]+\)|\S+)/g, '$2 + (($1/100) * $2)');
...
exports.PercentagePreprocessor = PercentagePreprocessor;
