...
exports.debugParseToAst = debugParseToAst;
...
const expressionComponents_1 = require("./expressionComponents");
 ...
 */
function parseLine(line, lineNumber = 1) {
    ...
    try {
        ...
        if (/^solve\b/i.test(trimmedLine)) {
            if ((0, expressionParser_1.needsExpressionEvaluation)(trimmedLine)) {
        ...
        const varParseResult = (0, variableParser_1.parseVariableAssignmentWithOptionalEvaluation)(trimmedLine);
        if (varParseResult.isValid && varParseResult.variableName) {
            if (varParseResult.showResult) {
                // Variable assignment with => (show result)
                return createCombinedAssignmentNode(varParseResult.variableName, varParseResult.expression || "", line, lineNumber);
            }
            else {
                // Variable assignment without => (just store)
                const rawValue = varParseResult.rawValue || varParseResult.value?.toString() || "";
                return createVariableAssignmentNode(varParseResult.variableName, rawValue, line, lineNumber);
            }
        }
        ...
        return createErrorNode(`Parse error: ${error instanceof Error ? error.message : "Unknown error"}`, "parse", line, lineNumber);
    }
}
/**
 * Parse a combined assignment line: variableName = expression =>
 */
function parseCombinedAssignment(line, lineNumber) {
    try {
        ...
        if (!variableName) {
            return createErrorNode("Missing variable name in combined assignment", "syntax", line, lineNumber);
        }
        if (!expression) {
            return createErrorNode("Missing expression in combined assignment", "syntax", line, lineNumber);
        }
        return createCombinedAssignmentNode(variableName, expression, line, lineNumber);
    }
    catch (error) {
        return createErrorNode(`Combined assignment parse error: ${error instanceof Error ? error.message : "Unknown error"}`, "parse", line, lineNumber);
    }
}
/**
 * Parse an expression line: expression => or unit expression
 */
function parseExpression(line, lineNumber) {
    try {
        const arrowIndex = line.indexOf("=>");
        if (arrowIndex === -1) {
        ...
        return createErrorNode(`Expression parse error: ${error instanceof Error ? error.message : "Unknown error"}`, "parse", line, lineNumber);
    }
}
/**
 * Factory functions for creating AST nodes
 */
function createPlainTextNode(raw, line) {
    return {
        type: "plainText",
        line,
        raw,
...
function createCommentNode(raw, line) {
    return {
        type: "comment",
        line,
        raw,
...
function createVariableAssignmentNode(variableName, rawValue, raw, line) {
    try {
        // Parse the raw value into a semantic value during AST creation
        const parsedValue = types_1.SemanticParsers.parseOrError(rawValue);
        return {
...
function createExpressionNode(expression, raw, line) {
    try {
        const isPercentageExpression = /%/.test(expression) ||
        ...
        let components = [];
        try {
            components = (0, expressionComponents_1.parseExpressionComponents)(expressionForComponents);
        }
        catch (error) {
            if (!isPercentageExpression && !(0, dateMath_1.looksLikeDateExpression)(expression)) {
                throw error;
            }
        }
        // Validate types early (without variable context yet)
        const hasFunction = components.some((component) => component.type === "function");
        if (!isPercentageExpression && !hasFunction && components.length > 0 && !(0, dateMath_1.looksLikeDateExpression)(expression)) {
            const typeError = (0, typeResolver_1.validateExpressionTypes)(components, new Map(), undefined, {
        ...
        return createErrorNode(`Expression parse error: ${error instanceof Error ? error.message : "Unknown error"}`, "parse", raw, line);
    }
}
function createCombinedAssignmentNode(variableName, expression, raw, line) {
    try {
        const isPercentageExpression = /%/.test(expression) ||
            /\bof\b/.test(expression) ||
            /\bon\b/.test(expression) ||
        ...
        return createErrorNode(`Combined assignment parse error: ${error instanceof Error ? error.message : "Unknown error"}`, "parse", raw, line);
    }
}
function createErrorNode(error, errorType, raw, line) {
    return {
        type: "error",
        line,
        raw,
...
function stripConversionSuffix(expression) {
...
function parseFunctionDefinition(trimmedLine, raw, line) {
    ...
    if (paramsResult && paramsResult.type === "error") {
        return paramsResult;
    }
    let components = [];
    try {
        components = (0, expressionComponents_1.parseExpressionComponents)(expression);
    }
    catch (error) {
        return createErrorNode(`Function body parse error: ${error instanceof Error ? error.message : "Unknown error"}`, "parse", raw, line);
    }
    return {
...
function splitTopLevelAssignment(line) {
    let depth = 0;
...
function parseFunctionParams(paramsRaw, raw, line) {
    if (!paramsRaw)
        return [];
    const params = [];
    const parts = [];
    ...
    for (const part of parts) {
        ...
        const defaultExpression = equalsIndex === -1 ? undefined : trimmed.substring(equalsIndex + 1).trim();
        ...
        if (defaultExpression) {
            try {
                defaultComponents = (0, expressionComponents_1.parseExpressionComponents)(defaultExpression);
            }
            catch (error) {
                return createErrorNode(`Invalid default for ${name}: ${error instanceof Error ? error.message : "Unknown error"}`, "parse", raw, line);
            }
        }
 ...
 */
function debugParseToAst(line) {
    return parseLine(line);
}
// Expose debug function in development
if (process.env.NODE_ENV === "development") {
    window.parseToAst = debugParseToAst;
}
