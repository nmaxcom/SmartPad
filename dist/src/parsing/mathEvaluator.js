...
exports.MathParser = exports.TokenType = void 0;
    ...
    evaluateFunction(name, args) {
        ...
        switch (lowerName) {
            case "sqrt":
                ...
                if (args[0] < 0) {
                    const error = new Error("Square root of negative number");
            ...
            case "max":
                if (args.length < 1)
                    throw new Error(`max expects at least 1 argument, got ${args.length}`);
                return Math.max(...args);
            case "min":
                if (args.length < 1)
                    throw new Error(`min expects at least 1 argument, got ${args.length}`);
                return Math.min(...args);
            case "sin":
                if (args.length !== 1)
                    throw new Error(`sin expects 1 argument, got ${args.length}`);
                return Math.sin(args[0]);
            case "cos":
                if (args.length !== 1)
                    throw new Error(`cos expects 1 argument, got ${args.length}`);
                return Math.cos(args[0]);
            case "tan":
                if (args.length !== 1)
                    throw new Error(`tan expects 1 argument, got ${args.length}`);
                return Math.tan(args[0]);
            case "log":
                if (args.length !== 1)
                    throw new Error(`log expects 1 argument, got ${args.length}`);
                if (args[0] <= 0) {
                    const error = new Error("Logarithm of non-positive number");
                    error.type = "INVALID_OPERATION";
                    throw error;
                }
            ...
            case "ln":
                if (args.length !== 1)
                    throw new Error(`ln expects 1 argument, got ${args.length}`);
                if (args[0] <= 0) {
                    const error = new Error("Logarithm of non-positive number");
                    error.type = "INVALID_OPERATION";
                    throw error;
            ...
            case "exp":
                if (args.length !== 1)
                    throw new Error(`exp expects 1 argument, got ${args.length}`);
 ...
 */
function evaluateMath(expression, variables = {}) {
    try {
