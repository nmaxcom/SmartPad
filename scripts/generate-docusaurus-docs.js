...
const SPEC_CATALOG = [
  ...
  },
  {
    ...
    focus: [
      ...
      "References stay linked to their source even when lines move.",
    ],
    syntax: [
      "Clicking the result value itself does not insert anything; that avoids accidental edits.",
      "Drag the chip itself into another expression to create a live reference.",
      "The copy icon copies the visible value. The menu exposes actions such as `Copy value` and plot or goal-seek suggestions when available.",
    ],
    checklist: [
      "Drag the chip when you want a value to stay connected to its source.",
      "Use the copy icon when you want plain text for another app.",
      "When a source breaks, check the warning chip before rewriting the whole sheet.",
    ],
    examples: {
      success: [
        {
  ...
  },
  {
    ...
    focus: [
      ...
      "Drag numeric values in the sheet to scrub assumptions and watch results update",
      "Persist key insights as detached `@view` blocks",
      "Keep text as source-of-truth while visuals stay synchronized",
    ],
    syntax: [
      "Numbers highlighted in the editor can be dragged horizontally to change them.",
      ...
      "Drag inside a chart to pan, scroll to zoom X, shift-scroll to pan, drag an axis to resize that viewport, and double-click to return to the computed view.",
    ],
    checklist: [
      "Scrub a number when you want to feel the sensitivity of a model before typing exact values.",
...
const stripUnneededExplicitTriggers = (code) =>
  code
    .split("\n")
    .map((line) => {
      if (/^\s*(make|solve)\b/i.test(line)) {
        return line;
      }
      return line.replace(/[ \t]+=>$/, "");
    })
...
const renderGuidePages = () => {
  ...
  return [
    ...
    },
    {
      ...
      body: [
        ...
        renderPlayground({
          ...
          description: "Drag `18` or `9` to explore the model.",
          code: [
            "principal = $12000",
            "annual return = 9%",
            "years = 18",
        ...
        "",
        renderPlayground({
          title: "A forecast with visible uncertainty",
          description:
            "Change either side of ±; SmartPad keeps the assumption and its consequences together.",
          code: [
            "visits = 10000 ± 2000",
            "conversion = 3% ± 0.5%",
            "price = 49 EUR",
    ...
    },
    {
      ...
      body: [
        ...
        renderPlayground({
          ...
          description: "Small patterns that cover most day-to-day sheets.",
          code: [
            "subtotal = $128",
            "tax = 8.5% on subtotal",
            "total = subtotal + tax",
        ...
        "",
        renderPlayground({
          title: "A reusable uncertain forecast",
          description:
            "The centre, interval, model steps, and final result all remain normal editable text.",
          code: [
            "visits = 10000 ± 2000",
            "conversion = 3% ± 0.5%",
            "price = 49 EUR",
        ...
        renderPlayground({
          ...
          description: "Use the List Spec Lab template for the longer tour.",
          code: [
            "prices = $12, $15, $9",
            "qty = 2, 1, 3",
        ...
        "",
        renderPlayground({
          title: "Shares as percentages",
          description: "Turn category values into readable shares.",
          code: [
            "rent = $1250",
            "utilities = $185",
            "internet = $75",
        ...
        "",
        renderPlayground({
          title: "Date and time math",
          description: "Calendar-aware lines stay readable in plain text.",
          code: [
            "launch = 2026-05-14",
            "launch + 21 days",
            "launch - 2026-05-01",
        ...
        "",
        renderPlayground({
          title: "Goal seek within limits",
          description:
            "Change the target to `13000 EUR` to see a clear no-feasible-solution explanation.",
          code: [
            "price = 50 EUR",
            "unit cost = 20 EUR",
            "orders = 200",
        ...
        "",
        renderPlayground({
          title: "A small editable table",
          description:
            "Change a source cell, add a row, or reuse a column in another calculation.",
          code: [
            "Orders:",
            "  item | qty | price",
            "  Notebook | 12 | 9 EUR",
        ...
        "",
        renderPlayground({
          title: "Linear algebra and complex values",
          description:
            "The Advanced Math template contains a longer guided example.",
          code: [
            "A = [[1, 2], [3, 4]]",
            "det(A)",
            "transpose(A)",
        ...
        "",
        renderPlayground({
          title: "Explicit symbolic math",
          description:
            "Every transformation stays visible and editable in the sheet.",
          code: [
    ...
    {
      file: "files-and-privacy.md",
      body: [
        "---",
        'title: "Files & Privacy"',
        "sidebar_position: 6",
        'description: "How SmartPad stores sheets, what stays local, and what to back up."',
        "---",
        "",
        "# Files & Privacy",
        "",
        "SmartPad is local-first. Your sheets are saved in your browser profile, and exports are plain Markdown you can keep elsewhere.",
        "",
        "## What stays local",
        "",
        "- Sheets are saved in browser storage on your machine.",
        "- SmartPad does not send sheet text, calculations, variables, or imported files to a SmartPad backend.",
        "- Browser profile resets, private browsing, device moves, or cleanup tools can remove local browser data.",
        "",
        "## Back up important work",
        "",
        "- Download one sheet as `.md` when you want a readable copy.",
        "- Use `Download All` before switching browsers, devices, or profiles.",
        "- Remove sensitive data before sharing screenshots, videos, or issue examples.",
        "",
        "## Current limits",
        "",
        "- There are no accounts, cloud sync, team workspaces, or collaboration yet.",
        "- Desktop builds are still a beta path until signed installers exist.",
        "- Currency conversions may depend on external FX providers or cached rates.",
        "",
        "## If something feels wrong",
        "",
        "Use [Troubleshooting](../troubleshooting) for calculation and syntax issues, or [Support](../support) when storage, import, export, or settings behavior looks wrong.",
        "",
      ].join("\n"),
    },
    {
...
const renderFeatureGuideExtraSections = (slug) => {
  if (slug === "result-chips-and-references") {
    return [
      "## What a chip is",
      "",
      "A result chip is the small pill SmartPad shows for a computed value. It is not just decoration: it is a handle for reusing the value safely.",
      "",
      "When you hover a result chip, SmartPad reveals three controls:",
      "",
      "- **Drag reuse**: drag the chip itself into another line to create a live reference to that result.",
      "- **Copy value**: copies the visible value as plain text.",
      "- **Actions menu**: opens commands such as `Copy value` and plot or goal-seek suggestions when available.",
      "",
      "Clicking the chip value itself is intentionally conservative. Drag the chip when you want SmartPad to reuse it in the sheet.",
      "",
      "> GIF/video marker: drag a result chip into another line, then hover a chip and point out the copy icon and menu.",
      "",
      "## References vs copied values",
      "",
      "A copied value is just text. A reference stays connected to the source result, so when the source changes, the dependent line can update too.",
      "",
      "Use a reference when the relationship matters. Use copy when you are pasting a value into another app or writing a one-off note.",
      "",
      "> GIF/video marker: drag the first chip icon into a formula, then change the original number and show the dependent result updating.",
      "",
    ];
  }

  if (slug === "plotting-and-dependency-views") {
    return [
      "## Scrub numbers to explore",
      "",
      "Many numbers in the editor are draggable. Click and drag a highlighted number left or right to change it without retyping. This is useful when you are exploring assumptions: rates, quantities, years, prices, thresholds, or any number that affects downstream results.",
      "",
      "Scrubbing edits the number in the sheet, so the rest of the model updates naturally. When a plot is connected to that value, the chart updates while you drag.",
      "",
      "> GIF/video marker: drag a number in a plotted model and show the result chip and plot changing live.",
      "",
      "## Move around charts",
      "",
      "Plots have lightweight navigation controls for exploring a relationship without editing the model:",
      "",
      "- **Pan**: drag inside the plot body to move the current view.",
      "- **Zoom X**: scroll over the chart to zoom around the pointer.",
      "- **Pan with wheel**: hold Shift while scrolling.",
      "- **Adjust an axis**: drag the X or Y axis to resize that viewport. The cursor changes on the draggable axis area.",
      "- **Reset view**: double-click the chart to return to the computed view.",
      "",
      "These interactions change the viewport, not the source domain. If you want the explored range to become part of the sheet, use the view/domain controls or write `view=`, `yview=`, `domain=`, or `ydomain=` on the `@view` line.",
      "",
    ];
  }
...
const main = () => {
  ...
  writeSidebar();
  console.log(`Generated docs: ${guidePageCount} guide pages, flat sidebar.`);
};
