 ...
 */
export class PercentageHelpers {
  /**
   * Calculate "X% of Y"
   * 
   ...
   */
  static of(percentValue: PercentageValue, baseValue: SemanticValue): SemanticValue {
   ...
   */
  static on(percentValue: PercentageValue, baseValue: SemanticValue): SemanticValue {
   ...
   */
  static off(percentValue: PercentageValue, baseValue: SemanticValue): SemanticValue {
   ...
   */
  static whatPercent(partValue: SemanticValue, baseValue: SemanticValue): PercentageValue {
    if (SemanticValueTypes.isNumber(partValue) && SemanticValueTypes.isNumber(baseValue)) {
   ...
   */
  static ofExpression(percent: number, baseExpr: string): string {
   ...
   * @returns The Math.js expression
   */
  static onExpression(percent: number, baseExpr: string): string {
   ...
   * @returns The Math.js expression
   */
  static offExpression(percent: number, baseExpr: string): string {
    return `${baseExpr} - ((${percent}/100) * ${baseExpr})`;
  }
}
