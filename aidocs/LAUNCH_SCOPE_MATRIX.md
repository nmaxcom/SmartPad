...
| First-run onboarding | Must ship as focused Decision Playground baseline | audit | `src/templates/decisionPlaygroundTemplate.ts`, `src/components/VariablePanel/VariablePanel.tsx`, `docs/Specs/BaselineComparison.spec.md`, `tests/unit/decisionPlaygroundTemplate.test.ts`, `tests/unit/variablePanelBaseline.test.tsx`, `tests/e2e/decision-playground-template.spec.ts`, `tests/e2e/variable-baseline-comparison.spec.ts` | Keep the user-confirmed focused model as first run; verify its capture-scrub-compare baseline flow, connected plot, and goal seek, while retaining Quick Tour as the secondary broad tour. |
