...
| First-run onboarding | Must ship as focused Decision Playground baseline | audit | `src/templates/decisionPlaygroundTemplate.ts`, `src/templates/quickTourTemplate.ts`, `tests/unit/decisionPlaygroundTemplate.test.ts`, `tests/e2e/decision-playground-template.spec.ts` | Verify that a fresh profile opens the focused live model without errors, that scrubbing/plot/goal-seek remain connected, and that the broad Quick Tour stays available as a secondary template. |
