...
| First-run onboarding | Must ship as focused Decision Playground baseline | audit | `src/templates/decisionPlaygroundTemplate.ts`, `src/components/ResultReferenceInteractionExtension.ts`, `docs/Specs/BaselineComparison.spec.md`, `tests/unit/decisionPlaygroundTemplate.test.ts`, `tests/unit/resultBaselineInteraction.test.ts`, `tests/e2e/decision-playground-template.spec.ts`, `tests/e2e/variable-baseline-comparison.spec.ts` | Keep the user-confirmed focused model as first run; verify its inline result-menu capture-scrub-compare flow, connected plot, and goal seek, while retaining Quick Tour as the secondary broad tour. |
