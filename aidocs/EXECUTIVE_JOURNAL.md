...
| P-2026-02-11-06 | Implement actual docs site and app docs link | Assistant | 2026-02-13 | done | Scaffolded the docs site, wired the app docs link, and aligned local docs publishing with the checked-in artifact |
...
| P-2026-06-06-01 | Manage SmartPad launch readiness in this thread | Assistant | TBD | in\_progress | Work through launch scope, stability, website/docs, desktop packaging, release operations, and public assets in priority order |
...
## Entry J-2026-06-04-04

*   Timestamp: 2026-06-04 13:02:05 CEST +0200 / 2026-06-04 11:02:05 UTC +0000
*   Summary:
    *   User requested a new fix batch focused on autocomplete edge cases plus nearby regressions in charts, currency parsing, legacy error formatting, tokenization, and variable hover highlighting.
    *   Assistant implemented autocomplete guards for click/caret activation, trailing whitespace exact-match suppression, and source-aware currency-vs-unit conversion suggestions.
    *   Assistant also fixed compact ISO currency literals, plot widget identity/current-marker rerendering, stale plot viewport override scoping, tooltip clamping, hover-to-highlight coverage, and standard warning formatting.
*   Decisions:
    *   Autocomplete remains typing-driven and does not open on pure selection/caret moves.
    *   Conversion autocomplete now routes currency sources to currency targets and unit/duration sources to unit targets.
    *   Plot viewport overrides are scoped by plot identity rather than only by line number.
    *   Human completion gate remains open until the user confirms the in-app behavior.
*   User directives:
    *   Find more autocomplete edge cases and convert old informative errors to the standard warning style while strengthening chart regression coverage.
*   Assistant commitments:
    *   Finish full machine gate validation, keep commits scoped, and report remaining risk if any gate fails.
*   Artifacts changed:
    *   `src/components/autocomplete/suggestions.ts`
    *   `src/components/autocomplete/AutocompleteExtension.ts`
    *   `src/components/PlotViewExtension.ts`
...
*   Validation:
    *   `npm run test:unit -- tests/unit/autocompleteSuggestions.test.ts --runInBand` ✅
    *   `npx playwright test tests/e2e/autocomplete.spec.ts --project=chromium --config=playwright.config.ts --workers=1` ✅
    *   `npm run test:unit -- tests/unit/currencyValue.test.ts tests/unit/semanticHighlightTokenization.test.ts --runInBand` ✅
    *   `npx playwright test tests/e2e/semantic-highlighting.spec.ts --project=chromium --config=playwright.config.ts --workers=1 -g "hovering a list variable"` ✅
    *   `npx playwright test tests/e2e/live-result.spec.ts --project=chromium --config=playwright.config.ts --workers=1 -g "warning reason|suppresses live errors"` ✅
    *   Plot e2e full run had one intermittent pre-existing wheel-zoom miss on first run; the same test passed isolated and all new plot regressions passed.
    *   `npx playwright test tests/e2e/plot-view-interactions.spec.ts --project=chromium --config=playwright.config.ts --workers=1` ✅
    *   `npm run docs:map` ✅
    *   `npm run docs:drift` ✅
    *   `npm run spec:test` ✅
...
*   Pending updates:
    *   `T-2026-06-04-01` remains `in_progress` pending user review/confirmation.
*   Risks/blockers:
    *   No machine blockers remain for this batch; `dist/index.html` was regenerated by build and should remain unstaged unless a release artifact update is explicitly requested.

## Entry J-2026-06-04-05

*   Timestamp: 2026-06-04 14:17 CEST
*   Summary:
    *   User reported a third autocomplete/polish batch: opaque function parameter names in the investment template, unnecessary `annual return` conversion, missing function hover highlighting, lingering autocomplete after exact identifier edits, hover-wrap flicker on result chips, wrong short-name hover matches, long-menu keyboard scroll issues, and weak `30kg to` conversion guidance.
    *   Assistant updated autocomplete exact-match suppression, unit-dimension filtering, active-option scrolling, function/variable hover boundary matching, result-chip hover layout stability, investment template naming, and dangling live conversion warnings.
*   Decisions:
    *   Autocomplete should not show an exact completed variable/function item while the user is still editing that token.
    *   Conversion autocomplete must respect the source unit dimension, not just broad unit-vs-currency categories.
    *   Result-chip hover controls should reserve stable width and change visibility only, avoiding layout-driven hover flicker.
    *   Live-result conversion errors should explain missing target units instead of falling back to `evaluation failed`.
    *   Human completion gate remains open until the user confirms the running-app behavior.
*   User directives:
    *   Fix the reported autocomplete, hover, template, and conversion edge cases.
*   Assistant commitments:
    *   Run focused unit/e2e validation plus full docs/spec/build/changed gates, then commit only assistant-authored files from this batch.
*   Artifacts changed:
    *   `src/components/Editor.tsx`
    *   `src/components/Editor.css`
    *   `src/components/VariableHoverExtension.ts`
    ...
    *   `tests/e2e/semantic-highlighting.spec.ts`
    *   `docs/Specs/proposed/autocomplete.md`
    *   `docs/Specs/LiveResult.spec.md`
    *   `docs/Specs/ResultChipsAndValueGraph.spec.md`
    *   `docs/spec-map.json`
    *   `aidocs/TODO_BACKLOG.md`
    *   `aidocs/EXECUTIVE_JOURNAL.md`
*   Validation:
    *   `npm run test:unit -- tests/unit/autocompleteSuggestions.test.ts tests/unit/investmentTemplate.test.ts --runInBand` ✅
    *   `npx playwright test tests/e2e/autocomplete.spec.ts tests/e2e/semantic-highlighting.spec.ts tests/e2e/live-result.spec.ts --project=chromium --config=playwright.config.ts --workers=1 -g "Autocomplete|hovering a list variable|function name|short variable|warning reason|dangling live unit|chip width"` ✅
    *   `npm run docs:map` ✅
    *   `npm run docs:drift` ✅
    *   `npm run spec:test` ✅
    *   `npm run spec:trust` ✅
...
*   Pending updates:
    *   `T-2026-06-04-01` remains `in_progress` pending user review/confirmation.
*   Risks/blockers:
    *   No machine blockers remain; `dist/index.html` was regenerated by build and should remain unstaged unless a release artifact update is explicitly requested.

## Entry J-2026-06-04-06

*   Timestamp: 2026-06-04 21:35 CEST
*   Summary:
    *   User rejected the previous result-chip flicker fix because reserving action width made chips visually too long.
    *   Assistant replaced the reserved-width approach with an out-of-flow absolute hover toolbar so the chip keeps its natural width while copy/menu actions remain available.
*   Decisions:
    *   Result-chip hover controls should not consume inline layout width.
    *   The anti-flicker contract is now: no chip width change, no forced line wrap, and actions render outside normal text flow.
*   User directives:
    *   Improve the flicker fix aesthetics after user confirmed the proposed direction.
*   Assistant commitments:
    *   Keep the change scoped to result-chip hover UI and preserve existing action behavior.
*   Artifacts changed:
    *   `src/components/Editor.css`
    *   `tests/e2e/live-result.spec.ts`
    *   `docs/Specs/ResultChipsAndValueGraph.spec.md`
    ...
    *   `aidocs/EXECUTIVE_JOURNAL.md`
*   Validation:
    *   `npx playwright test tests/e2e/live-result.spec.ts --project=chromium --config=playwright.config.ts --workers=1 -g "hover actions|chip width"` ✅
    *   `npm run docs:map` ✅
    *   `npm run docs:drift` ✅
    *   `npm run spec:test` ✅
    *   `npm run spec:trust` ✅
...
*   Pending updates:
    *   `T-2026-06-04-01` remains `in_progress` pending user review/confirmation.
*   Risks/blockers:
    *   No known blocker; `dist/index.html` was regenerated by build and should remain unstaged unless explicitly requested.

## Entry J-2026-06-04-07

*   Timestamp: 2026-06-04 22:02 CEST
*   Summary:
    *   User reported that the new hover toolbar has no space when Result Lane is enabled and asked to remove border and box shadow.
    *   Assistant changed Result Lane hover actions to open to the left of lane chips and removed border/shadow from the action cluster.
*   Decisions:
    *   Inline chips keep right-opening hover actions.
    *   Result Lane chips use left-opening hover actions to avoid colliding with the right lane boundary.
    *   Hover action clusters should be lightweight controls, not card-like popovers.
*   User directives:
    *   Make Result Lane toolbars open left and remove border/box shadow.
*   Assistant commitments:
    *   Keep the change scoped to hover action placement/chrome and preserve copy/menu behavior.
*   Artifacts changed:
    *   `src/components/Editor.css`
    *   `tests/e2e/live-result.spec.ts`
    *   `docs/Specs/ResultChipsAndValueGraph.spec.md`
    ...
    *   `aidocs/EXECUTIVE_JOURNAL.md`
*   Validation:
    *   `npx playwright test tests/e2e/live-result.spec.ts --project=chromium --config=playwright.config.ts --workers=1 -g "hover actions|chip width|result-lane hover"` ✅
    *   `npm run docs:map` ✅
    *   `npm run docs:drift` ✅
    *   `npm run spec:test` ✅
    *   `npm run spec:trust` ✅
...
*   Pending updates:
    *   `T-2026-06-04-01` remains `in_progress` pending user review/confirmation.
*   Risks/blockers:
    *   `dist/index.html` was regenerated by build and should remain unstaged unless explicitly requested.

## Entry J-2026-06-06-01

*   Timestamp: 2026-06-06 12:45 CEST / 2026-06-06 10:46 UTC
*   Summary:
    *   User set this thread's objective to preparing SmartPad for public launch while allowing feature work to continue in parallel elsewhere.
    *   Assistant reviewed the project operating manuals, current backlog, docs/build scripts, website/docs structure, settings surface, and release-related gaps.
    *   Assistant switched the active backlog lane to `project` and added launch-focused p0/p1 tasks covering scope, stability, settings/onboarding, website, desktop packaging, release operations, docs/support, and launch assets/analytics.
*   Decisions:
    *   Launch work should be managed primarily in the `project` lane, with explicit dependencies on `feature` and `maintenance` items.
    *   Public launch should proceed through scope freeze, stability audit, public surface/web/docs, packaging, and release operations rather than only UI polish.
    *   Electron/Tauri packaging remains undecided until a focused packaging decision memo compares fit for the current Vite/React app.
*   User directives:
    *   Treat publishing the app as the objective for this thread.
    *   Identify everything needed for launch, prioritize it, and suggest what to add/remove from the app and project.
*   Assistant commitments:
    *   Keep future work in this thread oriented around launch readiness and maintain scoped launch TODOs.
    *   Continue respecting the machine gate plus explicit human confirmation gate before marking launch tasks complete.
*   Artifacts changed:
    *   `aidocs/TODO_BACKLOG.md`
    *   `aidocs/EXECUTIVE_JOURNAL.md`
*   Pending items:
    *   `T-2026-06-06-01` Launch scope memo and must/should/later/remove decisions - Owner: Assistant - Due: TBD - Status: todo - Next: draft launch scope and map critical features to specs.
    *   `T-2026-06-06-02` Launch stability audit - Owner: Assistant - Due: TBD - Status: todo - Next: inventory core flow risks and convert uncovered p0 risks into tests or known limitations.
    *   `T-2026-06-06-03` Professional settings and onboarding baseline - Owner: Assistant - Due: TBD - Status: todo - Next: redesign settings IA and verify first-run/settings flows.
    *   `T-2026-06-06-04` Marketing website with signup - Owner: Assistant - Due: TBD - Status: todo - Next: create launch homepage plan with real app screenshots/video and signup integration decision.
    *   `T-2026-06-06-05` Desktop packaging pipeline - Owner: Assistant - Due: TBD - Status: todo - Next: decide Electron vs Tauri and prove packaged app behavior.
    *   `T-2026-06-06-06` Release operations - Owner: Assistant - Due: TBD - Status: todo - Next: add release checklist/versioning/changelog/intake flow.
    *   `T-2026-06-06-07` Public docs/support readiness - Owner: Assistant - Due: TBD - Status: todo - Next: audit beginner docs flow, accessibility, links, and runnable examples.
    *   `T-2026-06-06-08` Launch assets and privacy-respecting analytics - Owner: Assistant - Due: TBD - Status: todo - Next: capture assets and document tracking boundaries.
*   Risks/blockers:
...
*   Timestamp: 2026-06-07 00:00 CEST / 2026-06-06 22:00 UTC
*   Summary:
    *   User flagged that public docs assumed people already knew what result chips are.
    *   User noted outdated wording around clicking chips directly, which no longer matches the current drag handle, copy icon, and action menu behavior.
    *   User requested documentation for numeric scrubbing and placeholders for future GIF/video explanations.
*   Decisions:
    *   Result-chip docs now explain chips as the small result pills shown next to computed values.
    *   Docs now say direct chip-value clicks are conservative and should not be described as insertion behavior.
    *   Reuse flows are documented through the first chip icon drag handle, copy value icon, and actions menu.
    *   Numeric scrubbing is documented as dragging highlighted numbers left/right to explore assumptions.
    *   Public docs include visible GIF/video markers where future media would clarify the mechanics.
*   Artifacts changed in this block:
    *   `scripts/generate-docusaurus-docs.js`
    *   `website/docs/guides/getting-started.md`
    *   `website/docs/specs/result-chips-and-references.md`
    *   `website/docs/specs/plotting-and-dependency-views.md`
    *   `public/docs/` after local docs regeneration
    *   `aidocs/EXECUTIVE_JOURNAL.md`
*   Validation:
    *   `npm run docs:docusaurus:publish-local` passed.
    *   Spot-check confirmed result-chip docs explain drag handle, copy icon, actions menu, reference vs copied value, and video placeholders.
    *   Spot-check confirmed plotting docs explain numeric scrubbing and include a video placeholder.
    *   `npx playwright test tests/e2e/docs-ia.spec.ts --project=chromium --config=playwright.config.ts --workers=1` passed.
    *   `npm run docs:map`, `npm run docs:drift`, `npm run spec:test`, `npm run spec:trust`, `npm run build`, and `npm run verify:changed` passed.
*   Risks/blockers:
    *   Human gate is mandatory before committing this correction block.
    *   Unrelated worktree changes exist in `src/components/VariablePanel/TemplatePanel.tsx`, `tests/unit/plotViewEvaluator.test.ts`, and `tests/unit/templatePanelSheetCreation.test.tsx`; they were not touched by this docs block.

## Entry J-2026-06-07-02

*   Timestamp: 2026-06-07 01:05 CEST
*   Summary:
    *   User reported that the Currency FX template's "Plotting with FX" example produced a symbolic `total eur` value and a `No plottable data` chart.
    *   Assistant replaced the fragile `months = 0..12` + currency-list plot with function-backed FX series over a numeric domain: `usd_total(month)` and `eur_total(month)`.
*   Decisions:
    *   Template plot examples should avoid `0..12` assignment for month axes because that parses as a date in current assignment parsing.
    *   Currency FX plotting should use function-backed series with `domain=0..12` rather than currency lists as the x/y source.
*   User directives:
    *   Fix the broken example in the template.
*   Assistant commitments:
    *   Add regression coverage for the template content and the function-backed FX plot evaluator path.
*   Artifacts changed:
    *   `src/components/VariablePanel/TemplatePanel.tsx`
    *   `tests/unit/templatePanelSheetCreation.test.tsx`
    *   `tests/unit/plotViewEvaluator.test.ts`
...
*   Validation:
    ...
    *   `npm run build` ✅
*   Pending updates:
    *   Commit scoped template fix and run post-commit `verify:changed`.
*   Risks/blockers:
    *   `dist/index.html` was regenerated by build and should remain unstaged unless explicitly requested.

## Entry J-2026-06-07-03

*   Timestamp: 2026-06-07 05:47 CEST
*   Summary:
    *   User flagged that the public docs still did not read from a zero-knowledge user's perspective: Getting Started used "chip actions" before explaining chips, the sidebar had too many overlapping sections, and several examples needed logic/correctness review.
    *   Assistant reworked the Docusaurus public information architecture into a smaller user-facing path and removed public Feature Guides/spec pages from the Docusaurus sidebar/build output.
*   Decisions:
    *   Public Docusaurus docs should be the end-user manual, not a public rendering of internal specs.
    *   The public sidebar is now: Start Here, First Sheet, Core Interactions, Everyday Examples, Syntax Reference, Files & Privacy, Troubleshooting, Support.
    *   Result chips are introduced only after the user's first calculated line; chip controls, drag-reference behavior, copy, menu actions, and number scrubbing live in Core Interactions.
    *   Privacy, portability, backup guidance, and current user-facing limits are merged into Files & Privacy.
    *   Public interactive examples should pass through SmartPad's real evaluator unless they are explicitly marked as intentional error examples.
*   Artifacts changed:
    *   `scripts/generate-docusaurus-docs.js`
...
*   Validation:
    *   `npm run docs:docusaurus:publish-local` passed.
    *   `npm run docs:docusaurus:validate-examples` passed: 14 interactive examples validated.
    *   `npx playwright test tests/e2e/docs-ia.spec.ts --project=chromium --config=playwright.config.ts --workers=1` passed.
    *   `npm run docs:map`, `npm run docs:drift`, `npm run spec:test`, `npm run spec:trust`, `npm run build`, and `npm run verify:changed` passed.
*   Risks/blockers:
    *   Human gate is mandatory before committing this correction block.
    *   `dist/index.html` was regenerated by build and should remain unstaged unless explicitly requested.
    *   Unrelated template/plot worktree changes may still exist and must stay out of this docs commit.

## Entry J-2026-06-08-01

*   Timestamp: 2026-06-08 16:28 CEST
*   Summary:
    *   User approved the simpler docs order but asked whether the docs had become too sparse, especially because many detailed specs already exist as templates.
    *   Assistant extended the current Docusaurus docs block by adding an `Examples & Templates` bridge page and expanding `Syntax Reference` instead of reintroducing public spec pages.
    *   User specifically requested clearer unit documentation showing that SmartPad supports derived units such as volts and joules, not just a short family list.
*   Decisions:
    *   Docs teach the mental model and compact syntax reference.
    *   Templates remain the place for deeper executable capability tours and spec-like examples.
    *   Public docs should point users toward templates by task, without duplicating every spec as a page.
    *   Unit docs should explicitly name base units, derived units, dimensional math, SI prefixes, electrical examples, energy examples, and compound unit conversion.
*   Artifacts changed:
    *   `scripts/generate-docusaurus-docs.js`
    *   `tests/e2e/docs-ia.spec.ts`
    *   generated `website/docs/`
    *   generated `website/sidebars.ts`
    *   generated `public/docs/`
    *   `aidocs/EXECUTIVE_JOURNAL.md`
*   Validation:
    *   `npm run docs:docusaurus:publish-local` passed.
    *   `npm run docs:docusaurus:validate-examples` passed: 22 interactive examples validated.
    *   `npx playwright test tests/e2e/docs-ia.spec.ts --project=chromium --config=playwright.config.ts --workers=1` passed.
    *   `npm run docs:map`, `npm run docs:drift`, `npm run spec:test`, `npm run spec:trust`, `npm run build`, and `npm run verify:changed` passed.
*   Risks/blockers:
    *   Human gate is mandatory before committing this correction block.
...
## Entry J-2026-06-08-02

*   Timestamp: 2026-06-08 16:33 CEST
*   Summary:
    *   User rejected the `Examples & Templates` copy because it slipped back into internal/meta language such as "What belongs in the docs".
    *   Assistant corrected the generated public copy to speak directly to SmartPad users and registered a permanent docs-tone rule.
*   Decisions:
    *   Public Docusaurus pages must avoid metadocumentation/process framing and instead use user benefits, next actions, and task-oriented guidance.
    *   The forbidden pattern is now recorded in `aidocs/AI_DOCS_OPERATIONS.md`.
*   Artifacts changed:
    *   `aidocs/AI_DOCS_OPERATIONS.md`
    *   `scripts/generate-docusaurus-docs.js`
    *   regenerated `website/docs/`
    *   regenerated `public/docs/`
    *   `aidocs/EXECUTIVE_JOURNAL.md`
*   Validation:
    *   `npm run docs:docusaurus:publish-local` passed.
    *   `npm run docs:docusaurus:validate-examples` passed: 22 interactive examples validated.
    *   Search confirmed the rejected public phrases only remain in the internal docs-tone rule, not in generated public docs.
*   Risks/blockers:
    *   Human gate remains mandatory before committing this correction block.

## Entry J-2026-06-08-03

*   Timestamp: 2026-06-08 16:37 CEST
*   Summary:
    *   User requested removing `Examples & Templates` and declared the documentation valid after that correction.
    *   Assistant removed the page from the generator/sidebar, regenerated local public docs, and updated the docs IA test expectations.
*   Decisions:
    *   Final public docs IA for this block is: Start Here, First Sheet, Core Interactions, Everyday Examples, Syntax Reference, Files & Privacy, Troubleshooting, Support.
    *   The docs block is approved by the human gate after this correction.
*   Artifacts changed:
    *   `scripts/generate-docusaurus-docs.js`
    *   `tests/e2e/docs-ia.spec.ts`
    *   regenerated `website/docs/`
    *   regenerated `website/sidebars.ts`
    *   regenerated `public/docs/`
    *   `aidocs/EXECUTIVE_JOURNAL.md`
*   Validation:
    *   `npm run docs:docusaurus:publish-local` passed.
    *   `npm run docs:docusaurus:validate-examples` passed: 21 interactive examples validated.
    *   `npx playwright test tests/e2e/docs-ia.spec.ts --project=chromium --config=playwright.config.ts --workers=1` passed.
    *   Search confirmed `Examples & Templates` and `examples-and-templates` are no longer present in generated public docs or generator output.
    *   `npm run docs:map`, `npm run docs:drift`, `npm run spec:test`, `npm run spec:trust`, `npm run build`, and `npm run verify:changed` passed.
*   Risks/blockers:
    *   `dist/index.html` was regenerated by build and should remain unstaged.

## Entry J-2026-06-08-04

*   Timestamp: 2026-06-08 16:43 CEST
*   Summary:
    *   Continued the launch/publication goal after the public docs block was approved and committed.
    *   Inspected launch roadmap, scope matrix, docs/support audit, settings/onboarding brief, todo backlog, and current git state.
    *   Found launch-planning artifacts still referenced removed docs pages (`Known Limitations`, `Privacy and Portability`, `Syntax Playbook`) and still treated docs copy review as pending.
*   Decisions:
    *   Align launch planning with the approved docs IA from commit `7fc8867b`.
    *   Treat public docs/support/privacy as approved for now, with remaining work limited to future promotional-site links and homepage-to-docs-to-app journey verification.
    *   Keep Settings/first-run redesign as the next critical product block before screenshots, stability hardening, desktop beta, and promotional website work.
*   Artifacts changed:
    *   `aidocs/LAUNCH_EXECUTION_ROADMAP.md`
    *   `aidocs/LAUNCH_SCOPE_MATRIX.md`
    *   `aidocs/DOCS_SUPPORT_LAUNCH_AUDIT.md`
    ...
    *   `aidocs/EXECUTIVE_JOURNAL.md`
*   Validation:
    *   Searched the updated launch artifacts for obsolete docs page names and removed-path references.
    *   Diff review confirmed the changes are planning/status sync only; no product code or public docs were touched.
*   Pending:
    *   User confirmed this planning-sync block as valid on 2026-06-08 16:45 CEST; commit is authorized.
    *   Next implementation block should be Settings/first-run redesign planning with visible IA/design checkpoint before code changes.
*   Risks/blockers:
